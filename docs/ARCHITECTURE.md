...
│                                                             │
│ 根据配置的模型名称查表获取对应的 LLM Provider 实例              │
│                                                             │
│ 注册表（应用启动时初始化）：                                   │
...
│                                                             │
│ 从 SQLite 持久化存储中获取当前会话的历史消息：                   │
│   sessions 表: (agent_id, session_id) UNIQUE 约束            │
│   messages 表: 每条消息一行 (role, content)，按 id 自增排序    │
│                                                             │
│ system 消息不进入 session：仅 user/assistant/tool 被存储       │
│ system prompt 由 ContextBuilder 每轮构建                       │
│ 读取时按 SESSION_WINDOW_SIZE 取最近 N 条消息                   │
└──────────────────┬──────────────────────────────────────────┘
                   ▼
┌─────────────────────────────────────────────────────────────┐
...
SessionStore 是持久化会话管理模块，基于 SQLite 存储。
...
- **每条消息一行**：`role` 字段支持 `user`、`assistant`、`tool` 等，便于扩展新消息类型
- **双重隔离**：`(agent_id, session_id)` 复合键确保不同 Agent、同一 Agent 的不同会话之间完全隔离
- **窗口读取**：读取时按 `ORDER BY id DESC LIMIT window_size` 取最近 N 条，再反转为时间升序。写入不限制数量
- **角色过滤**：`system` 角色消息在存储时自动过滤，仅 `user`、`assistant`、`tool` 进入 messages 表。系统提示词由 ContextBuilder 每轮构建，不持久化到会话历史中
- **持久化**：进程重启后所有会话数据保留
- **零外部依赖**：不依赖 Redis、外部数据库，仅使用 SQLite (WAL 模式)
...
Agent 首次请求
  ...
  → 可选提供 session_id，未提供则自动生成 12 位 hex
  → 返回 {agent_id, agent_session_id}

后续请求 (携带相同 session_id)
  → 校验 session 在 sessions 表中存在（不存在 → 404）
  → 读取 messages 表中最近 window_size 条消息
  → 注入到上下文（system 消息之后，当前请求之前）
  → 请求完成后，将 user 消息 + assistant 回复逐条写入 messages 表

会话不存在时 append → SessionNotFoundError → 404
重复创建 session → SessionExistsError → 409
```
...
| 阶段 | 操作 |
|------|------|
| `--init` | 调用 `SessionDatabase.initialize(db_path)` 建库建表 |
| 启动 | `SessionDatabase.__init__` 打开连接 → 校验 schema → 启用 WAL |
| 运行时 | `execute()` / `commit()` 直接调用，不缓存不包装 |
| 关闭 | `await session_db.close()` 在 lifespan shutdown 中执行 |
...
按配置的模型名称路由到对应的 Provider 实例。当前只注册了 `deepseek-chat`。
